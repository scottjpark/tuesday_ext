...
const checkAccessToken = async () => {
    ...
    const url = 'https://tuesday.raphire.net/api/users/token/verify/';
    const data = {
        token: accessToken,
    };

    const tokenStatus = await $.ajax({ // eslint-disable-line
        method: 'POST',
        url,
        data,
    }).fail('caught');
...
const setStatusFields = (status) => {
    ...
    if (!(existingText.length > 0)) {
        ...
        statusText.style.fontFamily = TWITTER_FONT_FAMILY; // eslint-disable-line no-undef
...
// eslint-disable-next-line no-unused-vars
const addLoginFields = (parentNode, loginWidth) => {
    // Data Handling
    const handleFormSubmit = (e) => {
        ...
        const url = 'https://tuesday.raphire.net/api/users/token/';

        $.post(url, userData).done((response) => { // eslint-disable-line
            if (response.access) {
    ...
    loginBox.style.width = loginWidth;
    ...
    loginBox.style.fontFamily = TWITTER_FONT_FAMILY; // eslint-disable-line no-undef
    ...
    // Assembly
    parentNode.appendChild(loginBox);
